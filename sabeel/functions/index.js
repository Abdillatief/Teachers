...
exports.sendPushNotificationOnCreate = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    console.log("NOTIFICATION RECEIVED", { notificationId, data });
...
exports.sendPushNotificationOnUpdate = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (newData && newData.status === 'pending' && oldData.status !== 'pending') {
      console.log("NOTIFICATION RETRY TRIGGERED", { notificationId, newData });
...
/**
 * Fetches OneSignal API configuration from Firestore settings documents or process.env
 */
async function getOneSignalConfig(db) {
  let appId = process.env.ONESIGNAL_APP_ID || process.env.onesignalAppId || '61a2cc38-b4a8-4032-96ae-caa738df2ffd';
  let restApiKey = process.env.ONESIGNAL_REST_API_KEY || 
                   process.env.onesignalRestApiKey || 
                   process.env.oneSignalRestApiKey || 
                   process.env.ONESIGNAL_API_KEY || '';

  const checkDocData = (data) => {
    if (!data) return;
    if (data.onesignalAppId) appId = String(data.onesignalAppId).trim();
    if (data.appId && String(data.appId).includes('-')) appId = String(data.appId).trim();
    
    const keyCandidates = [
      data.onesignalRestApiKey,
      data.oneSignalRestApiKey,
      data.ONESIGNAL_REST_API_KEY,
      data.onesignalApiKey,
      data.oneSignalApiKey,
      data.restApiKey,
      data.apiKey
    ];
    for (const cand of keyCandidates) {
      if (!restApiKey && cand && typeof cand === 'string' && cand.trim().length > 0) {
        restApiKey = cand.trim();
        break;
      }
    }
  };
  ...
  try {
    const settingsDocs = ['academy', 'notifications', 'general', 'onesignal'];
    for (const docName of settingsDocs) {
      if (restApiKey && appId) break;
      try {
        const snap = await db.collection('settings').doc(docName).get();
        if (snap.exists) {
          checkDocData(snap.data());
        }
      } catch (innerErr) {
        // Continue checking next settings doc
      }
    }
  } catch (e) {
    console.warn('[OneSignal Config Warning] Error reading settings:', e);
  }
...
async function dispatchToOneSignal(config, payload) {
  ...
  const requestBody = {
    ...
    target_channel: "push",
    headings: {
      en: title || 'أكاديمية سبيل',
      ar: title || 'أكاديمية سبيل'
    },
    contents: {
      en: body || title || 'إشعار جديد من أكاديمية سبيل',
      ar: body || title || 'إشعار جديد من أكاديمية سبيل'
    },
    data: {
  ...
  // Targeting: Direct Firebase UID matching as external_user_ids
  ...
  console.log("ONESIGNAL PAYLOAD", requestBody);
  ...
  try {
    ...
    console.log("ONESIGNAL RESPONSE BODY:", resJson);
...
async function processPushNotification(notificationId, data) {
  ...
  try {
    if (recipientId === 'all') {
      ...
      if (userSnap.exists) {
        ...
        // Fallback: If recipientId is passed as UID directly, still attempt dispatching to that UID
        targetUsers.push({ id: recipientId, role: recipientRole || 'teacher' });
      }
    }
  } catch (err) {
  ...
  console.log("TARGET USERS", targetUsers.map(u => ({ id: u.id, role: u.role, name: u.name || u.displayName })));

  // 2. Prepare targeting: Every target user UID is ALWAYS added as OneSignal external_id!
  // This guarantees that any user who opens the Median APK receives push without gatekeeping.
  ...
  targetUsers.forEach(u => {
    if (u.id && !oneSignalExternalIds.includes(u.id)) {
  ...
  if (oneSignalExternalIds.length > 0 || oneSignalPlayerIds.length > 0) {
    ...
    if (osResult.success) {
      ...
      const errorDetail = osResult.error || osResult.errors || 'Unknown OneSignal error';
      deliveryLogs.push({
        channel: 'onesignal-native',
        status: 'failed',
        error: errorDetail,
      ...
      // Fallback: Record error in Firestore collection 'notification_errors'
      try {
        await db.collection('notification_errors').add({
          ...
          config: {
            ...
            hasApiKey: Boolean(osConfig.restApiKey)
          },
        ...
        console.error('[Notification Errors Collection Write Failed]:', errLog);
      }
    }
  }

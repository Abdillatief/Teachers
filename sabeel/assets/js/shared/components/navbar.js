...
import { isMedianApp, loginMedianOneSignal } from '../utils/medianBridge.js';
...
export function renderNavbar(title, userData = {}) {
  ...
  if (isMedianApp() && notifBanner) {
    // Native push is handled by OneSignal Native SDK in Median - hide web banner
  ...
  if (notifBtn && notifMenu) {
    notifBtn.addEventListener('click', async (e) => {
      ...
      // الاتصال بالمتصفح وطلب الإذن فوراً عند فتح قائمة الإشعارات في متصفحات الويب العادية
      if (!isMedianApp() && 'Notification' in window && Notification.permission === 'default') {
    ...
    if (btnEnableBrowserNotifs) {
      if (isMedianApp() || ('Notification' in window && Notification.permission === 'granted')) {
        if (notifBanner) notifBanner.style.display = 'none';
      }
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    loginMedianOneSignal(user, userRole, user.email).catch(err => console.warn("OneSignal login error:", err));

...
/**
 * Specifically detects if running inside Median Native Android WebView
 * @returns {boolean}
 */
export function isMedianAndroid() {
...
/**
 * Returns comprehensive runtime environment info
 */
export function getRuntimeEnvironment() {
  const isMedian = isMedianApp();
  const isAndroid = isMedianAndroid();
  const isIOS = isMedian && !isAndroid;
  const isStandalone = typeof window !== 'undefined' && (window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true);
  const isRealBrowser = !isMedian;

  return {
    isMedian,
    isMedianAndroid: isMedian && isAndroid,
    isMedianIOS: isMedian && isIOS,
    isRealBrowser,
    isPWA: isRealBrowser && isStandalone,
    platform: getAppPlatform(),
    supportsWebAuthn: isRealBrowser && typeof window !== 'undefined' && Boolean(window.PublicKeyCredential),
    supportsWebPush: isRealBrowser && typeof window !== 'undefined' && 'Notification' in window && 'serviceWorker' in navigator,
    supportsNativePush: isMedian,
    supportsNativeBiometrics: isMedian
  };
}
...
/**
 * Associates current authenticated user with OneSignal via Median Native Bridge & Web SDK
 * Sets OneSignal External User ID = Firebase User UID
 * Compatible with OneSignal User Model (v5+) and legacy Player Model (v3/v4).
 * 
 * @param {object|string} userOrUid - Firebase User object or UID string
 * @param {string} [role] - User role ('teacher' | 'admin')
 * @param {string} [email] - User email
 * @returns {Promise<{ oneSignalId: string|null, externalId: string|null, success: boolean }>}
 */
export async function loginMedianOneSignal(userOrUid, role = 'teacher', email = '') {
  if (!userOrUid) return { oneSignalId: null, externalId: null, success: false };

  const uid = (typeof userOrUid === 'string') ? userOrUid : (userOrUid.uid || '');
  if (!uid) return { oneSignalId: null, externalId: null, success: false };

  const userEmail = (typeof userOrUid === 'object' && userOrUid.email) ? userOrUid.email : (email || '');
  const userRole = role || (typeof userOrUid === 'object' && userOrUid.role) || localStorage.getItem('sabeel_user_role') || 'teacher';
  const platform = getAppPlatform();

  // 1. Store in localStorage for cross-page session continuity
  try {
    localStorage.setItem('sabeel_onesignal_external_id', uid);
    localStorage.setItem('sabeel_user_uid', uid);
    if (userRole) localStorage.setItem('sabeel_user_role', userRole);
  } catch (e) {}

  console.log(`[Median OneSignal] Associating User UID (${uid}) with OneSignal Native Bridge...`);

  const executeAssociation = () => {
    let linked = false;

    // A. Median / GoNative Native JS Bridge (OneSignal v5 - Recommended)
    try {
      if (window.median?.onesignal?.login) {
        window.median.onesignal.login({ externalId: uid });
        try { window.median.onesignal.login(uid); } catch (e) {}
        linked = true;
      }
      if (window.gonative?.onesignal?.login) {
        window.gonative.onesignal.login({ externalId: uid });
        try { window.gonative.onesignal.login(uid); } catch (e) {}
        linked = true;
      }
    } catch (e) {
      console.warn('[Median OneSignal] onesignal.login error:', e);
    }

    // B. Median / GoNative externalUserId.set syntax
    try {
      if (window.median?.onesignal?.externalUserId?.set) {
        window.median.onesignal.externalUserId.set({ externalId: uid });
        linked = true;
      }
      if (window.gonative?.onesignal?.externalUserId?.set) {
        window.gonative.onesignal.externalUserId.set({ externalId: uid });
        linked = true;
      }
    } catch (e) {
      console.warn('[Median OneSignal] externalUserId.set error:', e);
    }

    // C. Median / GoNative legacy setExternalUserId syntax (OneSignal v3/v4)
    try {
      if (window.median?.onesignal?.setExternalUserId) {
        window.median.onesignal.setExternalUserId({ externalId: uid });
        try { window.median.onesignal.setExternalUserId(uid); } catch (e) {}
        linked = true;
      }
      if (window.gonative?.onesignal?.setExternalUserId) {
        window.gonative.onesignal.setExternalUserId({ externalId: uid });
        try { window.gonative.onesignal.setExternalUserId(uid); } catch (e) {}
        linked = true;
      }
    } catch (e) {
    ...
    // D. Median URL Scheme fallback
    try {
      invokeMedianCommand('onesignal.login', { externalId: uid });
      invokeMedianCommand('onesignal.externalUserId.set', { externalId: uid });
      invokeMedianCommand('onesignal.setExternalUserId', { externalId: uid });
    } catch (e) {}

    // E. OneSignal Web SDK (Browser / PWA fallback if initialized)
    try {
      if (typeof window.OneSignal !== 'undefined') {
        if (typeof window.OneSignal.login === 'function') {
          window.OneSignal.login(uid).catch(() => {});
          linked = true;
        } else if (typeof window.OneSignal.setExternalUserId === 'function') {
          window.OneSignal.setExternalUserId(uid).catch(() => {});
          linked = true;
        }
      }
      ...
      console.warn('[OneSignal Web SDK] login error:', e);
    }

    // F. Request / Ensure Push Permission via Native Bridge
    try {
      if (window.median?.onesignal?.register) {
        window.median.onesignal.register();
      } else if (window.gonative?.onesignal?.register) {
        window.gonative.onesignal.register();
      } else if (window.median?.onesignal?.prompt) {
        window.median.onesignal.prompt();
      } else {
        invokeMedianCommand('onesignal.register');
      }
    } catch (e) {}

    // G. Set OneSignal User Tags (Role, UID, Email, Platform)
    const tags = {
      uid: uid,
      external_id: uid,
      role: userRole,
    ...
    // Exact verification log requested by user
    console.log("OneSignal user linked:", uid);
    console.log(`[Median OneSignal] Successfully associated external_id: ${uid}`);
  ...
  // Retry when Median library is ready or after brief delay
  if (typeof window !== 'undefined') {
    const onMedianReady = () => {
      console.log('[Median OneSignal] median_library_ready event triggered, re-linking UID:', uid);
      executeAssociation();
    };
    window.addEventListener('median_library_ready', onMedianReady, { once: true });
    document.addEventListener('median_library_ready', onMedianReady, { once: true });

    setTimeout(executeAssociation, 600);
    setTimeout(executeAssociation, 2000);
  }

  // Fetch OneSignal player/subscription info if available
  let oneSignalId = localStorage.getItem('sabeel_onesignal_player_id') || null;
  try {
    const info = await getMedianOneSignalInfo(3000);
    if (info?.userId || info?.playerId) {
      oneSignalId = info.userId || info.playerId;
    }
  } catch (e) {}
...
export const linkOneSignalUser = loginMedianOneSignal;
...
export async function initMedianOneSignal(user, role = 'teacher') {
  ...
  const res = await loginMedianOneSignal(user, role, user.email || '');
  return {
    oneSignalId: res.oneSignalId,
...
export function setMedianOneSignalTags(tags = {}) {
  try {
    ...
    if (typeof window.OneSignal !== 'undefined' && typeof window.OneSignal.sendTags === 'function') {
...
export function logoutMedianOneSignal() {
  try {
    ...
    } else if (window.gonative?.onesignal?.logout) {
      window.gonative.onesignal.logout();
    }

    if (window.median?.onesignal?.externalUserId?.delete) {
      window.median.onesignal.externalUserId.delete();
    }
    ...
    invokeMedianCommand('onesignal.logout');
    invokeMedianCommand('onesignal.externalUserId.delete');
    invokeMedianCommand('onesignal.removeExternalUserId');

    if (typeof window.OneSignal !== 'undefined' && typeof window.OneSignal.logout === 'function') {
...
/**
 * Prompts user for native push permission via OneSignal / Median Bridge
 */
export async function promptMedianPushPermission() {
  if (!isMedianApp()) return false;
  try {
    if (window.median?.onesignal?.register) {
      window.median.onesignal.register();
    ...
    console.warn('[MedianBridge] Error prompting native push:', e);
    return false;
  }
}
...
// Attach globally to window for immediate universal access across modules/pages
if (typeof window !== 'undefined') {
  window.loginMedianOneSignal = loginMedianOneSignal;
  window.logoutMedianOneSignal = logoutMedianOneSignal;
  window.linkOneSignalUser = loginMedianOneSignal;

  // Auto-link on page load if user UID is cached in localStorage
  try {
    const cachedUid = localStorage.getItem('sabeel_onesignal_external_id') || localStorage.getItem('sabeel_user_uid');
    if (cachedUid) {
      const cachedRole = localStorage.getItem('sabeel_user_role') || 'teacher';
      setTimeout(() => {
        loginMedianOneSignal(cachedUid, cachedRole);
      }, 500);
    }
  } catch (e) {}
}

...
export function calculateTeacherSalaryAdvanced(sessions, teacher, rangeType = 'all') {
  ...
  const bonuses = teacher ? parseFloat(teacher.salaryBonuses || 0) : 0;
  const deductions = teacher ? parseFloat(teacher.salaryDeductions || 0) : 0;
  ...
  (sessions || []).forEach(session => {
    ...
    // Check if session belongs to the current month or is a carried-over unarchived session from previous months
    const isCurrentMonth = Boolean(session.date && session.date.startsWith(currentYearMonth));
    ...
    // Range filter: in 'month' mode, include current month sessions PLUS any carried-over/unarchived sessions from previous months
    if (rangeType === 'month' && !isCurrentMonth && !isCarriedOver) {
      return;
    }
    ...
    if ((session.type === "trial" || session.sessionType === "trial" || session.isTrial === true) && (session.approved !== true || session.trialSubscribed !== true)) {
      return;
    }
    ...
    if (isApproved && (isCompleted || isUnexcusedAbsent)) {
      ...
      let sessionRate = rateInd;
      let sessionCost = 0;

      if (isGroup) {
        sessionRate = rateGrp;
        sessionCost = hours * rateGrp;
        result.groupHours += hours;
        result.groupSalary += sessionCost;
        result.groupSessionsCount++;
      } else {
        sessionRate = rateInd;
        sessionCost = hours * rateInd;
        result.individualHours += hours;
        result.individualSalary += sessionCost;
        result.individualSessionsCount++;
      }

      result.totalHours += hours;
      result.grossSalary += sessionCost;
      result.totalSessionsCount++;

      // Handle carry-over note if session is from previous months
      let carriedNote = '';
      let carriedMonthNum = '';
      let isSessionCarried = false;

      if (!isCurrentMonth && session.date && session.date < currentYearMonth) {
        isSessionCarried = true;
        const origMonth = (session.carriedOverFromMonth || session.date || '').substring(0, 7);
        const parts = origMonth.split('-');
        if (parts.length > 1) {
          carriedMonthNum = parseInt(parts[1], 10);
        }
        carriedNote = session.carriedOverNote || (carriedMonthNum ? `مرحّلة من شهر ${carriedMonthNum}` : 'مرحّلة من شهر سابق');
        
        result.carriedOverSessionsCount++;
        result.carriedOverSalary += sessionCost;
        result.carriedOverHours += hours;
        result.carriedOverSessions.push({
          ...session,
          carriedNote,
          carriedMonthNum
        });
      ...
      result.approvedSessions.push({
        id: session.id,
        studentName: session.studentName || session.trialStudentName || session.groupName || (isGroup ? 'مجموعة جماعية' : 'طالب'),
        date: session.date || '',
        time: session.time || session.sessionTime || '',
        duration: duration,
        hours: hours,
        isGroup: isGroup,
        type: isGroup ? 'group' : (session.type === 'trial' ? 'trial' : 'individual'),
        rate: sessionRate,
        cost: sessionCost,
        status: session.status,
        absenceType: session.absenceType || '',
        statusLabel: isCompleted ? 'حاضر ومكتمل ✅' : 'غياب غير مبرر 🔴',
        isCarriedOver: isSessionCarried,
        carriedOverNote: carriedNote,
        carriedOverMonthNum: carriedMonthNum,
        notes: carriedNote ? (session.notes ? `${session.notes} (${carriedNote})` : carriedNote) : (session.notes || '')
      });
    }
  });

  // Calculate Net Salary factoring in bonuses and deductions
  const net = Math.max(0, result.grossSalary + bonuses - deductions);
  result.netSalary = net;
  result.salary = net; // Keep backwards compatibility with callers expecting .salary

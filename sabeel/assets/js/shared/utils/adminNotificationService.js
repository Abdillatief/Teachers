...
export async function sendFeedbackRequestToTeachers({
  ...
  const docRef = await addDoc(collection(db, "notifications"), {
    recipientId: targetTeacherId || 'teachers',
    recipientRole: targetTeacherId === 'teachers' || targetTeacherId === 'all' ? 'all' : 'teacher',
    targetRole: 'teacher',
    type: 'feedback_request',
...
export async function replyToTeacherFeedback({
  ...
  if (!feedbackId || !teacherId || !replyMessage.trim()) {
    throw new Error('بيانات الرد غير مكتملة');
  }

  // Send feedback_reply notification to the teacher
  const notifTitle = `✉️ رد الإدارة على رأيك: ${topic || 'استطلاع الرأي'}`;
  const notifBody = `السلام عليكم أستاذنا (${teacherName || 'المعلم'})، بخصوص رأيكم حول (${topic}):\n\n"${replyMessage.trim()}"\n\nنشكركم جزيلاً على تفاعلكم ومقترحاتكم البناءة 🌸`;

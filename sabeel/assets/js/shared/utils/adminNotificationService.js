...
export async function notifyAdminSessionRecorded({
  ...
  // Trigger Push Notification via Cloudflare Worker Proxy
  sendPushNotification({
    title,
    body,
    recipientId: "admin",
    type: category,
    url: "/admin/sessions.html",
    data: { notifId: notifRef.id, sessionId: sessionId || '', teacherId: teacherId || '' }
...
export async function notifyAdminSessionNow({
  ...
  sendPushNotification({
    ...
    type: "session_ongoing",
    url: "/admin/sessions.html",
    data: { notifId: notifRef.id, reminderKey }
  }).catch(e => console.warn("Push error:", e));
}
...
export async function sendDirectReminderToTeacher({
  ...
  sendPushNotification({
    ...
    type: "admin_teacher_reminder",
    url: "/teacher/today-sessions.html",
    data: { notifId: docRef.id, studentName, scheduledTime }
  }).catch(e => console.warn("Push error:", e));
...
export async function sendFeedbackRequestToTeachers({
  ...
  sendPushNotification({
    ...
    recipientId: finalTarget,
    type: "feedback_request",
    url: "/teacher/feedback.html",
    data: { notifId: docRef.id, topic }
  }).catch(e => console.warn("Push error:", e));
...
export async function submitTeacherFeedback({
  ...
  sendPushNotification({
    ...
    type: "feedback_submitted",
    url: "/admin/feedback.html",
    data: { notifId: adminNotifRef.id, feedbackId: feedbackDocRef.id, teacherId }
  }).catch(e => console.warn("Push error:", e));
...
export async function replyToTeacherFeedback({
  ...
  sendPushNotification({
    ...
    type: "feedback_reply",
    url: "/teacher/feedback.html",
    data: { notifId: notifRef.id, feedbackId }
  }).catch(e => console.warn("Push error:", e));

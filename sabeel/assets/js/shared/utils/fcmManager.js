...
// Send Push Notification through Cloudflare Worker
try {
  await fetch("https://notification-api.prnccrft.workers.dev/", {
    method: "POST",
    headers: {
  ...
  console.log("Push sent to Cloudflare Worker successfully");

} catch (error) {
  console.error("Cloudflare Worker Push Error:", error);
}

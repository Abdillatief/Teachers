...
/**
 * Builds standard automated message templates
 */
export function generateWhatsAppMessage(templateType, { studentName = 'الطالب', teacherName = 'المعلم المشرف', pkg = null, bal = null, customData = {} }) {
  ...
  const used = pkg ? (pkg.usedLessons || 0) : 0;
  ...
  const totalPaid = pkg ? (pkg.totalPaid || 0) : (customData.paid || 0);
  const remainingDebt = Math.max(0, totalAmount - totalPaid);

  switch (templateType) {
    case 'overdraft':
      return `السلام عليكم ورحمة الله وبركاته 🌸\nتحية طيبة من أكاديمية سبيل لعلوم القرآن الكريم 📖\n\nنود إحاطة سيادتكم بأن رصيد حصص الطالب: *${studentName}*\nالمعلم المشرف: *${teacherName}*\nالباقة: *${pkgName}*\nقد نفد بالكامل وحالياً الرصيد (*${rem} حصة*) ⚠️\n\nنرجو من سيادتكم سرعة تجديد الباقة وسداد الرسوم المقررة (*${totalAmount} ج.م*) لضمان استمرار الحصص ومتابعة التسميع بانتظام.\n\nشاكرين ومقدرين حسن تعاونكم وحرصكم الكريم 🌿`;

    case 'reminder':
      return `السلام عليكم ورحمة الله وبركاته 🌸\nتحية طيبة من أكاديمية سبيل لعلوم القرآن الكريم 📖\n\nنود تذكير سيادتكم بموعد تجديد وسداد اشتراك باقة الطالب: *${studentName}*\nالمعلم المشرف: *${teacherName}*\nالباقة: *${pkgName}*\nالمبلغ المطلوب سداده: *${totalAmount} ج.م* ${remainingDebt > 0 && remainingDebt !== totalAmount ? `(المتبقي: ${remainingDebt} ج.م)` : ''}\n\nنرجو التكرم بالتحويل عبر الوسائل المعتمدة (فودافون كاش / إنستا باي) لضمان استمرار جدول الحصص.\n\nشاكرين ومقدرين حسن تعاونكم 🌿`;

    case 'statement':
      return `السلام عليكم ورحمة الله وبركاته 🌸\nأكاديمية سبيل لعلوم القرآن الكريم 📖\n\n📊 *تقرير الموقف المالي ورصيد الحصص*\nاسم الطالب: *${studentName}*\nالمعلم المشرف: *${teacherName}*\nالباقة المسندة: *${pkgName}*\n- إجمالي الحصص: ${total} حصة\n- الحصص المنفذة: ${used} حصة\n- الرصيد المتبقي: *${rem} حصة*\n- إجمالي القيمة: ${totalAmount} ج.م\n- المبلغ المسدد: ${totalPaid} ج.م\n- المبلغ المتبقي: *${remainingDebt} ج.م*\n\nنسعد دوماً بتواصلكم واستفساراتكم 🌿`;

    case 'receipt':
      return `السلام عليكم ورحمة الله وبركاته 🌸\nتحية طيبة من أكاديمية سبيل لعلوم القرآن الكريم 📖\n\n🧾 *إشعار استلام وسداد مالي معتمد*\nتم بحمد الله استلام وتوثيق دفعة مالية للطالب: *${studentName}*\nالمعلم المشرف: *${teacherName}*\nالمبلغ المستلم: *${customData.paidAmount || totalPaid || totalAmount} ج.م*\nرقم السند المالي: *#${customData.receiptNumber || 'REC-' + Math.floor(1000 + Math.random() * 9000)}*\n\nبارك الله فيكم وتقبل منا ومنكم صالح الأعمال 🌿`;

    case 'custom':
    default:
      return `السلام عليكم ورحمة الله وبركاته 🌸\nتحية طيبة من إدارة أكاديمية سبيل لعلوم القرآن الكريم بخصوص الطالب: *${studentName}*\nالمعلم المشرف: *${teacherName}*\n\n...`;
  }
}

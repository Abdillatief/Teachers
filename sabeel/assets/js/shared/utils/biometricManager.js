...
import { isMedianApp, promptMedianNativeBiometric, getAppPlatform } from './medianBridge.js';
...
export class BiometricManager {
  ...
  static async getDetailedStatus() {
    // 1. If running inside Median Mobile App Wrapper
    if (isMedianApp()) {
      return {
        supported: true,
        isMedian: true,
        ...
        isMobile: true,
        isIOS: getAppPlatform() === 'median-ios',
        isStandalone: true,
        reason: 'ready'
      };
    }

    // 2. Regular Web / PWA browser environment (WebAuthn)
  ...
  /**
   * Check if current browser and hardware supports User-Verifying Platform Authenticator
   * (Fingerprint / FaceID / TouchID / Median Native Biometrics)
   */
  static async isSupported() {
    try {
      // 1. Median App Wrapper always supports Native Biometrics (Android BiometricPrompt & iOS Touch ID / Face ID)
      if (isMedianApp()) {
        return true;
      }

      // 2. Standard Web Browser (WebAuthn API)
      if (
  ...
  static async registerBiometrics(userMeta) {
    ...
    // 1. Median App Native Biometrics Flow (Native Android/iOS Plugin)
    if (isMedianApp()) {
      console.log('[BiometricManager] Registering biometrics via Median Native Biometrics Bridge...');
      const promptTitle = 'تأكيد بصمة الجهاز لتطبيق سَبِيل 🔒';
      await promptMedianNativeBiometric(promptTitle);

      const credId = 'median_native_bio_' + Date.now();
    ...
    // 2. Standard Web Browser WebAuthn Flow
  ...
  static async verifyBiometrics() {
    ...
    // 1. Median App Native Biometrics Verification Flow
    if (isMedianApp() || savedUser.isMedian || credId.startsWith('median_native_bio')) {
      console.log('[BiometricManager] Verifying biometrics via Median Native Biometrics Bridge...');
      const promptTitle = 'تسجيل الدخول إلى تطبيق سَبِيل عبر البصمة 🔒';
      await promptMedianNativeBiometric(promptTitle);
      return savedUser;
    }

    // 2. Standard Web Browser WebAuthn Verification Flow
    const challenge = new Uint8Array(32);
    window.crypto.getRandomValues(challenge);

    const rawIdBuffer = this.base64ToBuffer(credId);
  ...
  static async checkAndPromptMobileSetup(userMeta) {
    // Only prompt on mobile / Median app / touch devices
      ...
      <div id="sabeelBiometricsModal" class="sabeel-bio-modal-overlay">
        <div class="sabeel-bio-modal-card">
          ...
          <p class="sabeel-bio-modal-desc">
            يبدو أن هذا المتصفح أو جهازك الحالي لا يدعم مستشعر البصمة والمقاييس الحيوية (Biometrics). يمكنك متابعة الدخول المباشر دائماً بكلمة المرور بكل سلاسة.
          </p>
  ...
  static showSetupPromptModal(userMeta) {
    ...
    const subTitleText = isMedian 
      ? 'يدعم تطبيق هاتفك ميزة البصمة ومستشعر الوجه (Face ID / Biometric)! هل ترغب في تفعيلها للدخول الفوري بلمسة واحدة دون كتابة كلمة المرور؟'
    ...
    const modalHtml = `
      <div id="sabeelBiometricsModal" class="sabeel-bio-modal-overlay">
        <div class="sabeel-bio-modal-card">
          ...
          <span class="sabeel-bio-badge">✨ أمان وسرعة فائقة ${isMedian ? '(تطبيق أصلي)' : ''}</span>

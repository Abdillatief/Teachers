...
class CentralDataStore {
  ...
  _startFirestoreListener(collectionKey, options = {}) {
    ...
    if (collectionKey === 'notifications') {
      q = query(collection(db, "notifications"), orderBy("createdAt", "desc"), limit(25));
    } else if (collectionKey === 'sessions' && options.currentMonthOnly) {
    ...
    try {
      const unsub = onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
          ...
          if (collectionKey === 'teachers') {
            if (data.role === 'teacher') {
              items.push({ uid: docSnap.id, id: docSnap.id, ...data });
            }
          } else {
        ...
        // Fallback for optional collections like expenses

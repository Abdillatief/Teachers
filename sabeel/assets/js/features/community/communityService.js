...
import { notifyCommunityAnnouncement, notifyTeacherFollowers } from '../../shared/services/notificationService.js';
...
export async function createCommunityPost({
  ...
  // 1. Admin/Official Announcements: Notify all teachers
  if (authorRole === 'admin' || authorRole === 'sub_admin' || postData.isOfficial || category === 'announcement') {
    notifyCommunityAnnouncement({
      ...
      console.warn('[CommunityService] Push dispatch warning:', err);
    });
  } else {
    // 2. Teacher Posts: Notify followers of this specific teacher (Phase 2 Requirement)
    notifyTeacherFollowers({
      authorId: postData.authorId,
      authorName: postData.authorName,
      postId: docRef.id
    }).catch(err => {
      console.warn('[CommunityService] Teacher follower notification warning:', err);
    });
  }

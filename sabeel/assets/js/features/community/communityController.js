...
export class CommunityController {
  ...
  buildPostCardHtml(post) {
    ...
    const authorAvatarHtml = avatarUrl
      ? `<img src="${avatarUrl}" alt="${post.authorName || 'معلم'}" style="width: 44px; height: 44px; border-radius: 50%; object-fit: cover; border: 2px solid ${authorBadgeColor}40; flex-shrink: 0;" onerror="this.onerror=null; this.style.display='none'; this.nextElementSibling.style.display='flex';">
    ...
    // Total Likes count
    const totalLikes = (reactions?.like?.length || 0) + (reactions?.love?.length || 0);
    ...
    return `
      <article id="postCard-${post.id}" class="card community-post-card ${isPinned ? 'pinned-post' : ''}" style="border-right: 4px solid ${cardBorderColor}; padding: 1.35rem; margin-bottom: 1.25rem;">
          ...
          <div style="display: flex; align-items: center; gap: 0.75rem;">
            ...
            <div>
              <div style="display: flex; align-items: center; gap: 0.45rem; flex-wrap: wrap;">
                <strong style="font-size: 0.98rem; color: var(--text-primary);">${post.authorName || 'عضو بالأكاديمية'}</strong>
                ...
                <span class="author-followers-badge" title="عدد المتابعين">
                  <i data-lucide="users" style="width: 12px; height: 12px;"></i>
                  <span data-follower-count-for="${authorId}">${followersCount}</span> متابع
                </span>
                ...
                ${canFollow ? `
                  <button type="button" 
                          ...
                          data-author-name="${post.authorName || 'المعلم'}" 
        ...
        <div class="community-toolbar-flex" style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--border-color); padding-top: 0.75rem; margin-top: 0.75rem; flex-wrap: wrap; gap: 0.65rem;">
          ...
          <!-- Reaction Buttons & Likes Summary Counter -->
          <div style="display: flex; align-items: center; gap: 0.45rem; flex-wrap: wrap;">
            ${reactionButtonsHtml}
            <div class="likes-summary-badge" title="إجمالي الإعجابات">
  ...
  attachPostEventListeners(post) {
    ...
    // 7. Follow/Unfollow Teacher button
      ...
      followBtn.addEventListener('click', async (e) => {
        ...
        try {
          const res = await toggleFollowTeacher(
            ...
            targetAuthorId,
            this.currentUser.name || 'معلم زميل'
          );
          ...
          // Optimistically update all follow buttons across the page
          this.updateFollowButtonsUI();

...
export function loadTeacherDashboardStats(teacher) {
  ...
  const computeAndRenderStats = () => {
    ...
    cachedSessions.forEach(session => {
      ...
      if (isCurrentMonth || isCarriedOver) {
        ...
        if (isApproved && (isCompleted || isUnexcusedAbsent)) {
          ...
          let rate = 0;
          if (isGroup) {
            rate = parseFloat(teacherData.hourlyRateGroup || teacherData.hourlyRateIndividual || teacherData.hourlyRate || 120);
          } else {
            rate = parseFloat(teacherData.hourlyRateIndividual || teacherData.hourlyRate || 100);
          }
    ...
    // Update monthly goal completion bar if available with dynamic target hours
    ...
    if (elProgress || elProgressText || elTargetSummary) {
      const targetHours = parseFloat(teacherData.monthlyTargetHours || teacherData.targetHours || 40);
  ...
  const unsubSessions = onSnapshot(sessionsQuery, (snapshot) => {
    cachedSessions = [];
    snapshot.forEach(doc => {
      cachedSessions.push({ id: doc.id, ...doc.data() });
    });
    computeAndRenderStats();
  }, (err) => console.error("Error listening to sessions stats:", err));

  // 3. Real-time listener on teacher's user document (reacts immediately when admin updates target hours or rates)
  const unsubTeacherDoc = onSnapshot(doc(db, "users", teacherId), (docSnap) => {
    if (docSnap.exists()) {
      teacherData = { ...teacherData, ...docSnap.data() };
      computeAndRenderStats();
    }
  }, (err) => console.warn("Teacher profile live listener warning:", err));

  // Return a combined unsubscribe function
  return () => {
    if (unsubStudents) unsubStudents();
    if (unsubSessions) unsubSessions();
    if (unsubTeacherDoc) unsubTeacherDoc();
  };
}

...
import { loginMedianOneSignal } from '../../shared/utils/medianBridge.js';
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Link OneSignal external ID immediately with newly created UID
        ...
        sendPushNotification({
          ...
          data: { notifId: adminNotifRef.id, teacherId: user.uid }
        }).catch(e => console.warn(e));

        // 5. Create a personal welcome notification in Firestore for the new teacher (to be viewed upon first login)
          ...
          try {
            ...
            if (querySnapshot.empty) {
              try {
                ...
                // Link OneSignal external ID with existing user UID
                loginMedianOneSignal(user, 'teacher', email);
                ...
                sendPushNotification({
                  ...
                  body: `المعلم السابق ${name} أرسل طلب انضمام جديد (استعادة ملف) وبانتظار المراجعة.`,
                  recipientId: "admin",
                  type: "teacher_restore_registration",
                  url: "/admin/teachers.html",
                  data: { notifId: adminNotifRef.id, teacherId: user.uid }
                }).catch(e => console.warn(e));

                const teacherWelcomeRef = await addDoc(collection(db, "notifications"), {
                ...
                sendPushNotification({
                  ...
                  body: `أهلاً بك أ. ${name}! تم تقديم طلب إعادة تفعيل حسابك كمعلم بنجاح وهو بانتظار الاعتماد.`,
                  recipientId: user.uid,
                  type: "welcome_teacher_restore",
                  url: "/index.html",
                  data: { notifId: teacherWelcomeRef.id }
                }).catch(e => console.warn(e));

...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderLogoAndName = (logoUrl, name, desc) => {
    ...
    const academyDesc = desc || 'نظام إداري متكامل لإدارة شؤون المعلمين والطلاب، تنظيم الحصص والجلسات، ومتابعة الاشتراكات والرواتب بدقة واحترافية.';
    
    if (loginLogoContainer) {
      const logoSrc = logoUrl || '/icons/icon-192.png';
      loginLogoContainer.innerHTML = `
        <div class="auth-brand-logo-wrap">
          <img src="${logoSrc}" class="academy-logo-img" alt="${academyName}">
        </div>
        <h1 class="auth-brand-title">${academyName}</h1>
        <p class="auth-brand-subtitle">${academyDesc}</p>
      `;
      if (window.lucide) window.lucide.createIcons();
    }
  };
  ...
  if (forgotPasswordForm) {
    forgotPasswordForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        Toast.success(`تم إرسال رابط إعادة تعيين كلمة المرور بنجاح إلى البريد الإلكتروني (${targetEmail}). يرجى مراجعة صندوق الوارد (Inbox) أو مجلد الرسائل غير المرغوب فيها (Spam).`);
        closeForgotModal();
    ...
    loginForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (role === 'admin') {
          ...
          // Check if system maintenance is active for teachers
          ...
          try {
            ...
            if (maintSnap.exists()) {
              ...
              if (acadSnap.exists()) {
                isTeacherMaintenance = acadSnap.data().maintenanceMode === true;
              }
            }
          } catch (mErr) {
          ...
          if (isTeacherMaintenance) {
            Toast.warning("تنبيه: النظام قيد الصيانة والتحديثات حالياً وسنعود بعد قليل.");
            setTimeout(() => {
        ...
        if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password' || error.code === 'auth/invalid-credential') {
          console.warn("Login attempt with invalid credentials:", error.message || error.code);
        } else {

...
import { isMedianApp, loginMedianOneSignal, logoutMedianOneSignal } from '../../shared/utils/medianBridge.js';
import { loginAndSyncOneSignal, logoutOneSignal } from '../../shared/services/oneSignalManager.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    // Immediately link OneSignal External ID with user.uid on persistent session detection
    loginMedianOneSignal(user, localStorage.getItem('sabeel_user_role') || 'teacher', user.email);
    ...
    try {
      ...
      // Re-link OneSignal with confirmed role
      loginMedianOneSignal(user, role, user.email);
      ...
      try {
        ...
        if (!userDoc || !userDoc.exists()) {
          ...
          if (email.includes('admin')) {
            await loginAndSyncOneSignal(user, 'admin').catch(e => console.warn(e));
        ...
        if (role === 'admin') {
          // Directly await OneSignal login & sync with admin role
          await loginAndSyncOneSignal(user, 'admin').catch(e => console.warn(e));
          ...
          // Directly await OneSignal login & sync with teacher role
          await loginAndSyncOneSignal(user, 'teacher').catch(e => console.warn(e));

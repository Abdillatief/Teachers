...
import { BiometricManager } from '../../shared/utils/biometricManager.js';
import { isMedianApp } from '../../shared/utils/medianBridge.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Biometric Quick Login & Prompt Integration
  const biometricLoginWrapper = document.getElementById('biometricLoginWrapper');
  ...
  const checkBiometricLoginAvailability = async () => {
    try {
      ...
      const isAvailable = await BiometricManager.isSupported();
      const isActive = BiometricManager.isBiometricLoginActive();

      if (isAvailable && (isActive || isMedian) && biometricLoginWrapper) {
        biometricLoginWrapper.style.display = 'block';
        const labelEl = document.getElementById('biometricBtnLabel');
        if (labelEl && isMedian) {
          labelEl.textContent = 'الدخول ببصمة الهاتف / Face ID';
        }
        if (window.lucide) window.lucide.createIcons();
      }
    } catch (e) {
  ...
  if (btnBiometricLogin) {
    btnBiometricLogin.addEventListener('click', async () => {
      try {
        const origText = btnBiometricLogin.innerHTML;
        btnBiometricLogin.disabled = true;
        btnBiometricLogin.innerHTML = `<i class="animate-spin" data-lucide="loader-2"></i> جاري مسح البصمة...`;
        if (window.lucide) window.lucide.createIcons();

        const userMeta = await BiometricManager.verifyBiometrics();
        Toast.success(`تم التحقق من البصمة بنجاح! مرحباً بك ${userMeta.name || ''} 🌸`);

        // Route user according to saved role
        if (userMeta.role === 'admin') {
          setTimeout(() => {
            window.location.href = '/admin/dashboard.html';
          }, 400);
        } else {
          triggerTeacherLoadingScreen(userMeta.name || userMeta.email, '/teacher/dashboard.html');
        }
      } catch (err) {
        console.warn("Biometric verification error:", err);
        Toast.warning(err.message || "فشلت عملية التحقق من البصمة، يمكنك كتابة كلمة المرور.");
      } finally {
        btnBiometricLogin.disabled = false;
        btnBiometricLogin.innerHTML = `
          ...
          <span id="biometricBtnLabel">تسجيل الدخول بالبصمة</span>
        `;
        if (window.lucide) window.lucide.createIcons();
      }
    });

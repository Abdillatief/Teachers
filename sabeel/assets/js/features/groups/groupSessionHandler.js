...
import { notifyAdminSessionRecorded } from '../../shared/utils/adminNotificationService.js';
...
export async function executeGroupSessionSubmit({
  ...
  for (const st of attendanceRecords) {
    ...
    await deductSessionCredit({
      ...
      status: isPresent ? "completed" : "student_absent",
      absenceType: !isPresent ? (st.absenceType || "unexcused") : "",
      actor: { uid: teacherId, name: teacherName || 'المعلم' }
    });
  }
  ...
  // Admin Notification with Color-coded metadata
  notifyAdminSessionRecorded({
    teacherId,
    teacherName,
    targetName: group.name,
    sessionType: 'group',
    scheduledTime: group.time || group.sessionTime || '',
    groupId: group.id

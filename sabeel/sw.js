/**
 * Sabeel Academy - High-Performance Progressive Web App (PWA) Service Worker
 * Version: 1.2.0
 * Architecture:
 *  - Network-First with Cache Fallback for dynamic application scripts & pages
 *  - Cache-First for static immutable media & icons
 *  - Zero-latency Offline Fallback
 *  - Safe exclusion of Firestore, Firebase Auth, and FCM endpoints
 */

const CACHE_NAME = 'sabeel-pwa-v1.2.0';
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      console.log('[Sabeel SW] Pre-caching core shell');
      ...
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (cacheName !== CACHE_NAME) {
            console.log('[Sabeel SW] Purging old cache:', cacheName);
...
self.addEventListener('fetch', (event) => {
  ...
  // B. Application JS modules: Network-First to ensure instant deployment updates
  if (url.pathname.endsWith('.js') && (url.pathname.startsWith('/assets/js/') || url.pathname.includes('/features/'))) {
    event.respondWith(
      fetch(request)
        .then((networkResponse) => {
          if (networkResponse && networkResponse.status === 200) {
  ...
  // C. Static Assets & Third-Party CDNs: Stale-While-Revalidate

...
export function renderNavbar(title, userData = {}) {
    ...
    <div class="navbar-actions" style="display: flex; align-items: center; gap: 0.75rem; position: relative;">
      ...
      <div style="position: relative; display: inline-block;" id="notifDropdownContainer">
        ...
        <div id="notifDropdownMenu" style="display: none; position: absolute; left: 0; top: 125%; width: 320px; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 8px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); z-index: 1000; padding: 1rem; direction: rtl;">
          ...
          <div id="browserNotifBanner" style="background: rgba(14, 165, 233, 0.08); border: 1px solid rgba(14, 165, 233, 0.25); border-radius: 6px; padding: 0.5rem; margin-bottom: 0.5rem; text-align: center;">
            <div style="font-size: 0.73rem; font-weight: 700; color: var(--text-primary); margin-bottom: 0.35rem;">🔔 إشعارات الحصص المنبثقة على المتصفح</div>
            <button id="btnEnableBrowserNotifs" class="btn btn-primary" style="font-size: 0.7rem; padding: 0.3rem 0.5rem; width: 100%; font-weight: 800; justify-content: center;">تفعيل التنبيهات المنبثقة 🔔</button>
          </div>
  ...
  if (!copyrightFooter) {
    ...
    copyrightFooter.innerHTML = `
      ...
      <span style="color: var(--primary-color); font-weight: 800; background: linear-gradient(135deg, #0ea5e9, #6366f1); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Sabilix</span>
    `;
        ...
        listContainer.innerHTML = items.map(item => {
          ...
          return `
            <div class="notif-item" data-id="${item.id}" style="background-color: ${bg}; padding: 0.6rem; border-radius: 6px; cursor: pointer; transition: background 0.2s; border: ${borderStyle}; display: flex; flex-direction: column; gap: 0.25rem; position: relative;">
              ...
              <p style="font-size: 0.75rem; color: var(--text-secondary); line-height: 1.3; margin: 0; text-align: right; padding-left: 20px;">${item.body || item.message || ''}</p>

...
import { openQuickGroupModal } from '../groups/quickGroupModal.js';
import { getAllGroups, addStudentToGroup } from '../groups/groupsManager.js';
...
function renderStudentsTable(filterText = "") {
  ...
  tbody.innerHTML = filtered.map(s => {
    ...
    const isGroup = s.studentType === 'group' || !!s.groupId;
    const typeBadge = isGroup 
      ? `<span class="badge" style="position:static; background-color: rgba(13, 148, 136, 0.12); color: var(--primary-color); font-weight: 700; font-size:0.78rem;">👥 ضمن مجموعة (${s.groupName || 'مجموعة'})</span>`
      : `<span class="badge" style="position:static; background-color: rgba(100, 116, 139, 0.1); color: var(--text-secondary); font-weight: 600; font-size:0.78rem;">👤 طالب فردي</span>`;
    ...
    return `
      <tr style="border-bottom: 2px solid var(--border-color);">
        ...
        <td style="padding: 1rem 0.75rem;">${typeBadge}</td>
        <td style="padding: 1rem 0.75rem;">${s.age || 'غير مسجل'} سنة</td>
        <td style="padding: 1rem 0.75rem;"><strong>${teacherName}</strong></td>
        <td style="padding: 1rem 0.75rem;">${s.sessionsCount || 0} حصة</td>
        <td style="padding: 1rem 0.75rem;"><div style="max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size:0.8rem; color:var(--text-secondary);" title="${s.notes || ''}">${s.notes || 'لا يوجد ملاحظات'}</div></td>
        <td style="padding: 1rem 0.75rem;"><strong style="color: var(--primary-color);">${(parseFloat(s.averageRating) || 5.0).toFixed(1)} / 5.0</strong></td>
        <td style="padding: 1rem 0.75rem;">${statusBadge}</td>
        <td style="padding: 1rem 0.75rem;">
          <div style="display: flex; gap: 0.35rem; flex-wrap: wrap; align-items: center;">
            <button class="btn btn-report-student" data-id="${s.id}" style="padding: 0.35rem 0.75rem; font-size: 0.8rem; font-weight: 800; color: var(--primary-color); background: linear-gradient(135deg, rgba(13, 148, 136, 0.15) 0%, rgba(13, 148, 136, 0.05) 100%); border: 1px solid rgba(13, 148, 136, 0.35); border-radius: 20px; display: inline-flex; align-items: center; gap: 0.35rem; cursor: pointer; box-shadow: 0 2px 6px rgba(13, 148, 136, 0.08);"><i data-lucide="award" style="width:14px;height:14px;"></i> بطاقة تقرير</button>
            <button class="btn btn-secondary btn-edit-student" data-id="${s.id}" style="padding: 0.35rem 0.65rem; font-size: 0.8rem;"><i data-lucide="edit-3"></i> تعديل</button>
            <button class="btn btn-danger btn-delete-student" data-id="${s.id}" style="padding: 0.35rem 0.65rem; font-size: 0.8rem;"><i data-lucide="trash-2"></i> أرشيف</button>
          </div>
        </td>
      </tr>
    `;
  ...
  gridContainer.innerHTML = filtered.map(s => {
    ...
    const isGroup = s.studentType === 'group' || !!s.groupId;
    const typeBadge = isGroup 
      ? `<span class="badge" style="position:static; background-color: rgba(13, 148, 136, 0.12); color: var(--primary-color); font-weight: 700; font-size:0.75rem;">👥 ضمن مجموعة (${s.groupName || 'مجموعة'})</span>`
      : `<span class="badge" style="position:static; background-color: rgba(100, 116, 139, 0.1); color: var(--text-secondary); font-weight: 600; font-size:0.75rem;">👤 طالب فردي</span>`;
    ...
    return `
      <div style="background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 12px; padding: 1.25rem; display: flex; flex-direction: column; justify-content: space-between; gap: 1rem; box-shadow: 0 2px 5px rgba(0,0,0,0.03);">
        <div>
          <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 0.5rem; margin-bottom: 0.75rem;">
            <div>
              ...
              <div style="margin-top: 0.25rem;">${typeBadge}</div>
            </div>
            ${statusBadge}
          </div>
        ...
        <div style="display: flex; gap: 0.5rem; border-top: 1px solid var(--border-color); padding-top: 0.75rem; flex-wrap: wrap;">
          <button class="btn btn-report-student" data-id="${s.id}" style="flex: 1 1 100%; justify-content: center; padding: 0.5rem 0.85rem; font-size: 0.82rem; gap: 0.4rem; background: linear-gradient(135deg, rgba(13, 148, 136, 0.16) 0%, rgba(13, 148, 136, 0.05) 100%); color: var(--primary-color); border: 1px solid rgba(13, 148, 136, 0.38); border-radius: 12px; font-weight: 800; display: inline-flex; align-items: center; cursor: pointer; box-shadow: 0 2px 6px rgba(13, 148, 136, 0.08);">
            <i data-lucide="award" style="width: 16px; height: 16px; color: var(--primary-color);"></i> إصدار بطاقة تقرير شهري للوالدين 📜
          </button>
  ...
  document.querySelectorAll('.btn-report-student').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = e.currentTarget.dataset.id;
      const studentObj = studentsCached.find(st => st.id === id);
      if (studentObj) {
        openStudentReportModal(id, studentObj);
      }
    });
  });
  ...
  if (approveForm) {
    approveForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // 2. إنشاء خطة اشتراك معتمدة جديدة للشهر الحالي
...
async function loadGroupsForAddModal(selectEl, teacherId = null) {
  if (!selectEl) return;
  try {
    const allGroups = await getAllGroups();
    const filtered = teacherId ? allGroups.filter(g => g.teacherId === teacherId) : allGroups;
    if (filtered.length === 0) {
      selectEl.innerHTML = `<option value="">لا توجد مجموعات حالياً (اضغط + إنشاء مجموعة)</option>`;
    } else {
      selectEl.innerHTML = `<option value="">-- اختر مجموعة --</option>` + filtered.map(g => `<option value="${g.id}">${g.name} (${g.teacherName || 'معلم'})</option>`).join('');
    }
  } catch (err) {
    console.error(err);
  }
...
function openAddStudentModalForm() {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="modal-card">
        ...
        <form id="addStudentForm">
          ...
          <!-- نوع الطالب (فردي / ضمن مجموعة) -->
          <div class="form-group" style="margin-top: 1rem; background: var(--bg-primary); padding: 0.85rem; border-radius: 8px; border: 1px solid var(--border-color);">
            <label style="font-weight: 800; color: var(--primary-color); display: block; margin-bottom: 0.5rem;">نوع الطالب</label>
            <div style="display: flex; gap: 1.5rem; align-items: center;">
              <label style="display: flex; align-items: center; gap: 0.35rem; cursor: pointer; font-weight: 700; font-size: 0.88rem;">
                <input type="radio" name="addStudentType" value="individual" checked id="addTypeIndividual"> ○ طالب فردي
              </label>
              <label style="display: flex; align-items: center; gap: 0.35rem; cursor: pointer; font-weight: 700; font-size: 0.88rem;">
                <input type="radio" name="addStudentType" value="group" id="addTypeGroup"> ○ ضمن مجموعة
              </label>
            </div>

            <div id="addStudentGroupSelectContainer" style="display: none; margin-top: 0.85rem; padding-top: 0.85rem; border-top: 1px dashed var(--border-color);">
              <div style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap;">
                <div style="flex: 1; min-width: 200px;">
                  <label for="addStudentGroupSelect" style="font-size: 0.8rem; font-weight: 700; color: var(--text-secondary); display: block; margin-bottom: 0.25rem;">اختر المجموعة التعليمية</label>
                  <select id="addStudentGroupSelect" class="form-control" style="width: 100%;">
                    <option value="">-- اختر مجموعة --</option>
                  </select>
                </div>
                <button type="button" class="btn btn-secondary" id="btnCreateNewGroupInStudentModal" style="margin-top: 1.25rem; white-space: nowrap; font-weight: 700; font-size: 0.8rem; gap: 0.25rem;">
                  <i data-lucide="plus"></i> + إنشاء مجموعة جديدة
                </button>
              </div>
            </div>
          </div>
    ...
    // Dynamic group visibility toggle & loader
    const radIndividual = modal.querySelector('#addTypeIndividual');
    const radGroup = modal.querySelector('#addTypeGroup');
    const groupContainer = modal.querySelector('#addStudentGroupSelectContainer');
    const groupSelect = modal.querySelector('#addStudentGroupSelect');
    const teacherSelect = modal.querySelector('#addStudentTeacher');

    const toggleGroupSelect = async () => {
      if (radGroup.checked) {
        groupContainer.style.display = 'block';
        await loadGroupsForAddModal(groupSelect, teacherSelect.value);
      } else {
        groupContainer.style.display = 'none';
      }
    };

    radIndividual.addEventListener('change', toggleGroupSelect);
    radGroup.addEventListener('change', toggleGroupSelect);
    teacherSelect.addEventListener('change', () => {
      if (radGroup.checked) loadGroupsForAddModal(groupSelect, teacherSelect.value);
    });

    // Create group on the fly inside modal
    modal.querySelector('#btnCreateNewGroupInStudentModal').addEventListener('click', () => {
      const selectedTeacherId = teacherSelect.value;
      const selectedTeacherObj = teachersCached.find(t => t.uid === selectedTeacherId);
      openQuickGroupModal({
        teacherId: selectedTeacherId || null,
        teacherName: selectedTeacherObj ? selectedTeacherObj.name : '',
        onGroupCreated: async (newGroup) => {
          await loadGroupsForAddModal(groupSelect, selectedTeacherId);
          if (groupSelect && newGroup?.id) {
    ...
    modal.querySelector('#addStudentForm').addEventListener('submit', async (e) => {
      ...
      const isGroup = modal.querySelector('#addTypeGroup').checked;
      const groupId = isGroup ? groupSelect.value : '';
      const selectedGroupOpt = isGroup && groupSelect.selectedIndex >= 0 ? groupSelect.options[groupSelect.selectedIndex] : null;
      const groupName = (isGroup && selectedGroupOpt && groupSelect.value) ? selectedGroupOpt.text.split(' (')[0] : '';
      ...
      await TransactionProtector.executeProtected({
        ...
        actionFn: async () => {
          ...
          const studentData = {
            name,
            age,
            studentType: isGroup ? 'group' : 'individual',
          ...
          if (isGroup && groupId) {
            await addStudentToGroup(groupId, docRef.id, groupName);
          }
...
function setupEventListeners() {
  ...
  const btnOpenAddGroup = document.getElementById('openAddGroupModalFromStudents');
  if (btnOpenAddGroup) {
    btnOpenAddGroup.addEventListener('click', () => {
      openQuickGroupModal();
    });
  }

...
import { assignPackageToStudent } from '../../shared/utils/creditManager.js';
...
function renderStudentsTable(filterText = "") {
  ...
  tbody.innerHTML = paginatedStudents.map(s => {
    ...
    const pkgLabel = s.packageName ? `<span class="badge badge-primary" style="position:static; margin-top:0.25rem;">${s.packageName}</span>` : '';
    ...
    if (s.remainingLessons !== undefined) {
      ...
      if (rem < 0) {
        creditBadge = `<span class="badge badge-danger" style="position:static; margin-top:0.25rem;">عجز حصص: ${rem}</span>`;
      } else if (rem === 0) {
        creditBadge = `<span class="badge badge-danger" style="position:static; margin-top:0.25rem;">0 حصة (منتهي)</span>`;
      } else if (rem <= 3) {
        creditBadge = `<span class="badge badge-warning" style="position:static; margin-top:0.25rem;">${rem} حصص متبقية</span>`;
      } else {
        creditBadge = `<span class="badge badge-success" style="position:static; margin-top:0.25rem;">${rem} حصة متبقية</span>`;
      }
    }
  ...
  gridContainer.innerHTML = paginatedStudents.map(s => {
    ...
    const daysLabel = s.days && Array.isArray(s.days) && s.days.length > 0 ? s.days.join('، ') : (s.sessionDays ? s.sessionDays.join('، ') : 'غير محدد');
    const timeLabel = s.time || s.sessionTime || 'غير محدد';
    const pkgLabel = s.packageName ? `<span class="badge badge-primary" style="position:static;">${s.packageName}</span>` : '';
    ...
    if (s.remainingLessons !== undefined) {
      const rem = parseInt(s.remainingLessons) || 0;
      if (rem < 0) {
        creditBadge = `<span class="badge badge-danger" style="position:static;">عجز: ${rem}</span>`;
      } else if (rem === 0) {
        creditBadge = `<span class="badge badge-danger" style="position:static;">0 حصة</span>`;
      } else if (rem <= 3) {
        creditBadge = `<span class="badge badge-warning" style="position:static;">${rem} متبقية</span>`;
      } else {
        creditBadge = `<span class="badge badge-success" style="position:static;">${rem} متبقية</span>`;
      }
    }

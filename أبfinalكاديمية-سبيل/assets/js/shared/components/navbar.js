    ...
    onSnapshot(collection(db, "notifications"), (snapshot) => {
      ...
      snapshot.forEach(docSnap => {
        ...
        if (isAdmin) {
          ...
          // EXCLUDES parent-only personal notifications
          if (!isParentOnly) {
            if (isAdminOnly || data.recipientId === 'all' || data.recipientId === user.uid) {
              isRecipient = true;
            }
          }
        } else if (isParent) {
          // Parent receives only parent notifications
          if (isParentOnly || data.parentPhone === user.phoneNumber || data.recipientId === user.uid) {
            isRecipient = true;
          }
        } else if (isTeacher) {
          // Teacher receives only teacher notifications
          if (!isAdminOnly && !isParentOnly) {
            const isDirectRecipient = data.recipientId === user.uid;
            const isTeacherRecipient = (data.recipientId === 'teachers' || data.recipientId === 'all' || data.recipientRole === 'teacher');
            const isTeacherIdMatch = data.teacherId === user.uid;

            if (isDirectRecipient || isTeacherIdMatch || isTeacherRecipient) {
              isRecipient = true;
            }
          }
        } else {
      ...
      if (listContainer) {
        ...
        listContainer.innerHTML = items.map(item => {
          ...
          const bg = isRead ? 'transparent' : (meta.bgSubtle || 'var(--bg-secondary)');
          const borderRight = `4px solid ${meta.borderColor || 'var(--primary-color)'}`;
          const borderStyle = isRead ? '1px solid var(--border-color)' : `1px solid ${meta.borderColor || 'var(--primary-color)'}`;
          const fontWeight = isRead ? 'normal' : 'bold';
          const indicator = isRead ? '' : `<span style="display:inline-block; width:7px; height:7px; background:${meta.color || 'var(--primary-color)'}; border-radius:50%; margin-left:4px;"></span>`;
          
          return `
            <div class="notif-item" data-id="${item.id}" style="background-color: ${bg}; padding: 0.65rem 0.75rem; border-radius: 8px; cursor: pointer; transition: all 0.2s; border: ${borderStyle}; border-right: ${borderRight}; display: flex; flex-direction: column; gap: 0.3rem; position: relative;">
              <div style="display: flex; align-items: center; justify-content: space-between; gap: 0.5rem; direction: rtl; padding-left: 24px;">
                <span style="font-weight: ${fontWeight}; font-size: 0.82rem; color: var(--text-primary); display: flex; align-items: center; gap: 0.35rem;">
                  <i data-lucide="${meta.icon || 'bell'}" style="width: 15px; height: 15px; color: ${meta.color || 'var(--primary-color)'}; flex-shrink: 0;"></i>
              ...
              <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 0.15rem; padding-left: 24px;">
              ...
              <button class="delete-single-notif-btn" data-id="${item.id}" style="position: absolute; left: 6px; top: 12px; background: none; border: none; cursor: pointer; padding: 0.25rem; display: flex; align-items: center; justify-content: center; border-radius: 4px;" title="حذف الإشعار">
...
export function showNotificationModalDetails(notification) {
  ...
  modalEl.innerHTML = `
    <div class="card modal-card" style="width: 100%; max-width: 480px; position: relative; border-top: 3px solid var(--primary-color); border: 1px solid var(--border-color); box-shadow: var(--shadow-xl);">
      ...
      <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
        <div style="width: 40px; height: 40px; background: var(--primary-light); color: var(--primary-color); border-radius: var(--border-radius-sm); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
          <i data-lucide="${isPasswordRequest ? 'key-round' : 'bell'}" style="width: 20px; height: 20px;"></i>
        </div>
        <div>
      ...
      <div style="margin-bottom: 1.25rem;">
        ...
        <p style="font-size: 0.85rem; color: var(--text-secondary); line-height: 1.6; white-space: pre-line; background: var(--bg-primary); border: 1px solid var(--border-color); padding: 0.75rem; border-radius: var(--border-radius-sm); margin: 0;">${notification.body || 'لا توجد تفاصيل إضافية'}</p>
      </div>

...
import { collection, onSnapshot, doc, updateDoc, arrayUnion, query, where, getDocs } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
...
export function renderNavbar(title, userData = {}) {
  ...
  auth.onAuthStateChanged((user) => {
    ...
    // Check if user is admin to show the direct link to broadcast
    ...
    }).catch(err => console.warn("Failed to read user role for navbar link:", err));
    ...
    onSnapshot(collection(db, "notifications"), (snapshot) => {
      ...
      snapshot.forEach(docSnap => {
        ...
        if (data.recipientId === 'all' || data.recipientId === user.uid) {

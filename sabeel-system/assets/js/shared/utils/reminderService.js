...
/**
 * Triggers a native system browser notification
 */
export function sendNativeNotification(title, body, url = '/teacher/today-sessions.html') {
  ...
  try {
    ...
    for (const docSnap of snap.docs) {
      ...
      if (diffMinutes >= 0 && diffMinutes <= 15) {
        ...
        // 3. Play audio chime sound, Toast & Native Push Notification
        if (user.uid === student.teacherId || user.uid === student.uid) {
          playNotificationChime();
          Toast.warning(`⏰ تنبيه حصة: ${title} - ${student.name}`);
          sendNativeNotification(title, user.uid === student.teacherId ? teacherMessage : studentMessage);
        }
      }
    }
  } catch (err) {
...
/**
 * Initializes auto reminder interval when app is active & registers service worker
 */

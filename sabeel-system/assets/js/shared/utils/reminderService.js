...
import { notifyAdminUnrecordedSession } from './adminNotificationService.js';
...
export async function checkAndSendSessionReminders() {
  ...
  try {
    ...
    for (const docSnap of snap.docs) {
      ...
      // --- TRIGGER 5: Missing Session Report Alert / Admin Overdue Alert ---
      if (diffMinutes <= -15) {
        const isRecorded = await isSessionRecordedToday(student.teacherId, student.id, todayDateStr);
        if (!isRecorded) {
          // إشعار الأدمن بحصة طالب فردية غير مسجلة
          notifyAdminUnrecordedSession({
            teacherId: student.teacherId,
    ...
    // --- CHECK GROUPS SCHEDULE FOR OVERDUE UNRECORDED SESSIONS ---
    try {
        ...
        if (diffGroupMins <= -15) {
          ...
          if (gSnap.empty) {
            notifyAdminUnrecordedSession({
              ...
              teacherName: group.teacherName || 'المعلم المعتمد',
      ...
      console.warn("Group schedule check error:", gErr);
    }
  } catch (err) {

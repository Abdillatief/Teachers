    ...
    menuSections = [
      ...
      },
      {
        ...
        items: [
          ...
          { id: 'blackbox', label: 'بلاك بوكس 📦', icon: 'box', path: 'blackbox.html' }
        ]
      }
    ];
...
function initSidebarAdminTodaySessions() {
  ...
  const updateSidebarUI = () => {
    ...
    groupsTodayCache.forEach(grp => {
      ...
      let statusBadge = '<span style="font-size:0.65rem; font-weight:800; color:#6366f1; background:rgba(99,102,241,0.15); padding:2px 6px; border-radius:4px;">لم تتم ⏳</span>';
      if (recordedGS) {
      ...
      if (recorded) {
        if (recorded.status === 'completed') {
          ...
          if (recorded.postponedFrom === todayStr && recorded.date !== todayStr) {
            statusBadge = `<span style="font-size:0.65rem; font-weight:800; color:#d97706; background:rgba(217,119,6,0.15); padding:2px 6px; border-radius:4px;">مؤجلة إلى (${recorded.date}) ⏳</span>`;
          } else {
  ...
  unsubscribeSidebarSessions = onSnapshot(collection(db, "sessions"), (sessSnap) => {
    ...
    sessSnap.forEach(d => {
      ...
      if (data.date === todayStr || data.postponedFrom === todayStr) {
        sessionsTodayCache.push({ id: d.id, ...data });
      }
    });
    updateSidebarUI();
  ...
  unsubscribeSidebarGroups = onSnapshot(collection(db, "groups"), (groupSnap) => {
    groupsTodayCache = [];
    groupSnap.forEach(d => {
      const grp = { id: d.id, ...d.data() };
    ...
    updateSidebarUI();
  }, (err) => console.warn("Error fetching groups for sidebar:", err));
}

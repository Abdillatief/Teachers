...
export async function recordGroupAttendance({
  ...
  // 3. Create individual student session records in 'sessions' collection for backend reports, stats, salary
  const sessionPromises = attendees.map(async (att) => {
    ...
    const isCalculatedSalary = isCompleted || (att.absenceType === 'unexcused');
    ...
    const sessionData = {
      ...
      notes: att.note ? `${generalNotes ? generalNotes + ' | ' : ''}ملاحظة الطالب: ${att.note}` : (generalNotes || `حصة مجموعة (${groupName})`),

    ...
    for (const ym of Object.keys(monthGroups)) {
      ...
      for (const teacherId of Object.keys(teacherGroups)) {
        ...
        if (roundedSalary > 0) {
          ...
          // Send individual notification to teacher
          ...
          batch.set(teacherNotifRef, {
            ...
            message: `تم إعادة ضبط السجل للشهر الجديد وأرشفة مستحقاتك المالية لشهر (${ym}) بمبلغ (${roundedSalary.toLocaleString()} ج.م) عن (${sessionsCount} حصة / ${totalHours.toFixed(1)} ساعة). المستحقات معتمدة وبانتظار تحويل الإدارة.`,
            type: "salary_archived",
            read: false,
            createdAt: serverTimestamp()
          });
        } else {
          // Even if salary is 0, archive sessions from previous month to reset active counters
          sessions.forEach(sess => {
            const sessRef = doc(db, "sessions", sess.id);
      ...
      // Send consolidated Admin notification if salaries were archived

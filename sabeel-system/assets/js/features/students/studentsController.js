...
function populatePackageOptions() {
  ...
  if (packagesCached.length > 0) {
    pkgSelect.innerHTML = `<option value="">-- اختر الباقة التعليمية المناسبة للطالب --</option>` +
      packagesCached.map(p => `<option value="${p.id}" data-price="${p.price}" data-name="${p.name}" data-sessions="${p.sessions || p.sessionsCount || 8}" data-duration="${p.duration || p.sessionDuration || 60}">${p.name} (${p.price} ج.م - ${p.sessions || p.sessionsCount || 8} حصص)</option>`).join('');
  } else {
    pkgSelect.innerHTML = `
...
function renderPendingStudentsTable() {
  ...
  tbody.innerHTML = pendingStudentsCached.map(s => {
    ...
    const subTypeBadge = s.subscriptionType === 'group' || s.groupId || s.requestedGroupId
      ? `<span class="badge" style="background: rgba(14, 165, 233, 0.12); color: var(--primary-color); font-weight: 700; font-size: 0.72rem; position: static;">👥 مجموعة ${s.requestedGroupName ? `: ${s.requestedGroupName}` : ''}</span>`
      : `<span class="badge" style="background: rgba(16, 185, 129, 0.12); color: var(--success); font-weight: 700; font-size: 0.72rem; position: static;">👤 فردية</span>`;
...
function renderStudentsTable(filterText = "") {
  ...
  tbody.innerHTML = paginatedStudents.map(s => {
    ...
    const subTypeBadge = (s.subscriptionType === 'group' || s.groupId)
      ? `<span class="badge" style="position:static; background: rgba(14, 165, 233, 0.12); color: var(--primary-color); font-weight: 700; margin-top:0.25rem;">👥 مجموعة (${s.groupName || 'دراسي'})</span>`
      : `<span class="badge" style="position:static; background: rgba(100, 116, 139, 0.12); color: var(--text-secondary); font-weight: 600; margin-top:0.25rem;">👤 فردي</span>`;
    ...
    return `
      <tr style="border-bottom: 2px solid var(--border-color);">
        <td style="padding: 1rem 0.75rem;">
          ...
          <div style="display:flex; gap:0.3rem; flex-wrap:wrap; margin-top:0.25rem;">${subTypeBadge} ${pkgLabel}</div>
        </td>
  ...
  gridContainer.innerHTML = paginatedStudents.map(s => {
    ...
    const subTypeBadge = (s.subscriptionType === 'group' || s.groupId)
      ? `<span class="badge" style="position:static; background: rgba(14, 165, 233, 0.12); color: var(--primary-color); font-weight: 700;">👥 مجموعة (${s.groupName || 'دراسي'})</span>`
      : `<span class="badge" style="position:static; background: rgba(100, 116, 139, 0.12); color: var(--text-secondary); font-weight: 600;">👤 فردي</span>`;
    ...
    return `
      <div style="background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 12px; padding: 1.25rem; display: flex; flex-direction: column; justify-content: space-between; gap: 1rem; box-shadow: 0 2px 5px rgba(0,0,0,0.03);">
        <div>
          ...
          ${pkgLabel ? `<div style="margin-bottom: 0.5rem; display:flex; gap:0.4rem; flex-wrap:wrap;">${subTypeBadge} ${pkgLabel}</div>` : `<div style="margin-bottom: 0.5rem;">${subTypeBadge}</div>`}
